							...
							<div class="content">
								<ul style="list-style-type: none;">
									<li><input type="url" id="url"
										value="<%=InetAddress.getLocalHost().getCanonicalHostName()%>:<%=request.getLocalPort()%><%=this.getServletConfig().getServletContext().getContextPath()%>/SonarViolationChangeReport?groupId=<%=groupId%>&artifactId=<%=artifactId%>"
										size="54"></li>
						...
						<script type="text/javascript">
							$(document).ready(function() {
									$("#request").click(function(){
										window.open("<%=InetAddress.getLocalHost().getCanonicalHostName()%>:<%=request.getLocalPort()%><%=this.getServletConfig().getServletContext().getContextPath()%>/SonarViolationChangeReport?groupId=<%=groupId%>&artifactId=<%=artifactId%>");
							})
						});
						</script>
						<div class="panel">
